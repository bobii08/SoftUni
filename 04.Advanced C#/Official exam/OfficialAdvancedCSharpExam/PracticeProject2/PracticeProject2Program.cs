namespace Problem4
{
    ...
    internal class Events
    {
        private static void Main()
        {
            ...
            SortedDictionary<string, SortedDictionary<string, List<string>>> data =
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                if (regex.IsMatch(line) && match.Groups.Count == 4)
                {
                    ...
                    {
                        data[townName] = new SortedDictionary<string, List<string>>();
                    }

                    if (!data[townName].ContainsKey(personName))
                    {
                        data[townName][personName] = new List<string>();
                    }
