...
namespace BookingSystem.Controllers
{
    ...
    public class UsersController : Controller
    {
        ...
        public IView Register(string username, string password, string confirmPassword, string role)
        {
            ...
            var existingUser = Data.RepositoryWithUsers.GetByUsername(username);
            ...
            var user = UserFactory.CreateUser(username, password, userRole);
        ...
        {
            foreach (var user in Data.RepositoryWithUsers.GetAll())
            {
                if (string.IsNullOrEmpty(user.Username) || (CurrentUser != null && CurrentUser.Username == user.Username))
                {
